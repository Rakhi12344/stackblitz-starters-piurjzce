...
// sorted by growth
app.get('/stock/sort/growth', (req, res) => {
  let { order } = req.query;
  let filterdStocks = [...stocks];
  if (order === 'desc') {
    filterdStocks.sort((a, b) => b.growth - a.growth);
  } else {
    filterdStocks.sort((a, b) => a.growth - b.growth);
  }
  res.json({ stocks: filterdStocks });
});
...
// Endpoint to filter stocks based on exchange
app.get('/stocks/filter/exchange', (req, res) => {
  ...
  if (!exchange) {
    return res
      .status(400)
      .json({ error: 'Please provide an exchange parameter (nse or bse).' });
  }
  ...
  if (filteredStocks.length === 0) {
    return res
      .status(404)
      .json({ message: `No stocks found for exchange: ${exchange}` });
  }
